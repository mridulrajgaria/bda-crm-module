...
const SAMPLE_NOTIFICATIONS = [
  { id: 1, message: 'New lead assigned to you', sub: 'CNC Parts — Tata Motors', time: '2 min ago', read: false, icon: TrendingUp, color: 'bg-indigo-100 text-indigo-600' },
  ...
  { id: 3, message: 'Lead status updated to Won', sub: 'Hydraulic Cylinders — JSW Steel', time: '3 hr ago', read: true, icon: TrendingUp, color: 'bg-purple-100 text-purple-600' },
  { id: 4, message: 'Follow-up reminder', sub: 'Gear Assemblies — Bajaj Auto', time: 'Yesterday', read: true, icon: Bell, color: 'bg-yellow-100 text-yellow-600' },
];
  ...
  return (
    <header className="h-16 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between px-4 lg:px-6 sticky top-0 z-10">
      ...
      <div className="flex items-center gap-4">
        <button onClick={onMenuClick} className="lg:hidden p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800 text-gray-600 dark:text-gray-300">
          <Menu size={20} />
        </button>
        <div>
      ...
      <div className="flex items-center gap-1.5">
        ...
        <div ref={searchRef} className="relative">
          ...
          {searchOpen && (
            <div className="absolute right-0 top-full mt-2 w-80 bg-white dark:bg-gray-900 rounded-xl shadow-xl border border-gray-100 dark:border-gray-700 z-30">
              ...
              <div className="py-1 max-h-64 overflow-y-auto">
                ...
                {!searching && searchResults.map((r) => (
                  ...
                  >
                    <span className={`text-[10px] px-1.5 py-0.5 rounded font-semibold ${r.type === 'lead' ? 'bg-indigo-100 text-indigo-600' : 'bg-green-100 text-green-600'}`}>
                      {r.type}
                    </span>
        ...
        <div className="relative">
          ...
          {notifOpen && (
            <>
              ...
              <div className="absolute right-0 top-full mt-2 w-80 bg-white dark:bg-gray-900 rounded-xl shadow-xl border border-gray-100 dark:border-gray-700 z-20">
                ...
                <div className="divide-y divide-gray-50 dark:divide-gray-800 max-h-72 overflow-y-auto">
                  {notifications.map((n) => (
                    <div key={n.id} className={`flex items-start gap-3 px-4 py-3 hover:bg-gray-50 dark:hover:bg-gray-800 ${!n.read ? 'bg-indigo-50/40 dark:bg-indigo-900/20' : ''}`}>
        ...
        <div className="relative">
          ...
          {dropOpen && (
            <>
              ...
              <div className="absolute right-0 top-full mt-1 w-48 bg-white dark:bg-gray-900 rounded-xl shadow-lg border border-gray-100 dark:border-gray-700 py-1 z-20">
                <div className="px-4 py-2 border-b border-gray-100 dark:border-gray-700">
                  <p className="text-sm font-semibold text-gray-800 dark:text-gray-100">{user?.name}</p>
                  <p className="text-xs text-gray-400 capitalize">{user?.role}</p>
                </div>
                <Link to="/profile" onClick={() => setDropOpen(false)} className="w-full flex items-center gap-2 px-4 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800">
                  <User size={15} /> Profile
                </Link>
                <Link to="/settings" onClick={() => setDropOpen(false)} className="w-full flex items-center gap-2 px-4 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800">
